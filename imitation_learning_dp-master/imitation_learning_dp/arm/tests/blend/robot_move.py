...
if __name__ == '__main__':
    ...
    rrt_map = RRTMap(area=[(0.3, -0.3), (0.7, 0.3)], obstacles=obstacles)

    start = [0.3, -0.3]
    goal = [0.7, 0.3]
    rrt_parameter = RRTParameter(start=[0.3, -0.3], goal=[0.7, 0.3], expand_dis=0.1, animation=False)
    rrt_planner = RRT2DPlanner(rrt_map=rrt_map, rrt_parameter=rrt_parameter)

    R0 = SO3.RPY(-np.pi, 0.0, -np.pi / 2)

    path_parameters = rrt_planner.get_path_parameters(R0)
    radii = [1.0 for i in range(len(path_parameters) - 1)]
    ...
    for i, si in enumerate(ss):
        Ti = blend_planner.interpolate(si)
