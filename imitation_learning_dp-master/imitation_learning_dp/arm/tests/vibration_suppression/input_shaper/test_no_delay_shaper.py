...
from arm.vibration_suppression import ZVShaper, ZVDShaper, NoDelayShaper

Td = 0.5364
zeta = 0.1


class TestNoDelayShaper(TestCase):
    def test_shape(self):
        robot = UR5e()
        q0 = np.zeros(6)
        robot.set_joint(q0)
        ...
        timestep = 0.001
        time_step_num = round(total_time / timestep) + 1
        times = np.linspace(0, total_time, time_step_num)

        time1 = 2
        time1s = np.arange(0, time1 + timestep, timestep)
        T0 = robot.get_cartesian()
        t0 = T0.t
        R0 = SO3(T0.R)
        t1 = t0 + np.array([0.2, -0.4, 0.5])
        R1 = SO3(T0.R)
        ...
        velocity_parameter = QuinticVelocityParameter(time1)
        ...
        trajectory = np.zeros((time1s.size, 3))
        for i, timei in enumerate(time1s):
            planner_interpolate = trajectory_planner.interpolate(timei)
            trajectory[i, :] = planner_interpolate.t

        raw_trajectory = np.zeros((time_step_num, 3))
        zv_shaped_trajectory = np.zeros_like(raw_trajectory)
        zvd_shaper_trajectory = np.zeros_like(raw_trajectory)
        no_delay_shaped_trajectory = np.zeros_like(raw_trajectory)

        raw_trajectory[:trajectory.shape[0], :] = trajectory
        raw_trajectory[trajectory.shape[0]:, :] = trajectory[-1, :]

        omega_d = 2 * np.pi / Td
        zv_shaper = ZVShaper(omega_d, zeta, timestep)
        zv_shaped_trajectory[:time1s.shape[0] + zv_shaper.delay_count, :] = zv_shaper.shape(trajectory)
        zv_shaped_trajectory[time1s.shape[0] + zv_shaper.delay_count:, :] = zv_shaped_trajectory[
                                                                            time1s.shape[0] + zv_shaper.delay_count - 1,
                                                                            :]
        zvd_shaper = ZVDShaper(omega_d, zeta, timestep)
        zvd_shaper_trajectory[:trajectory.shape[0] + zvd_shaper.delay_count, :] = zvd_shaper.shape(trajectory)
        zvd_shaper_trajectory[trajectory.shape[0] + zvd_shaper.delay_count:, :] = zvd_shaper_trajectory[
                                                                                  trajectory.shape[
                                                                                      0] + zvd_shaper.delay_count - 1,
                                                                                  :]
        no_delay_shaped = NoDelayShaper(zvd_shaper)
        no_delay_shaped_trajectory[:trajectory.shape[0] + no_delay_shaped.delay_count, :] = no_delay_shaped.shape(
            trajectory)
        no_delay_shaped_trajectory[trajectory.shape[0] + no_delay_shaped.delay_count:, :] = no_delay_shaped_trajectory[
                                                                                            trajectory.shape[
                                                                                                0] + no_delay_shaped.delay_count - 1,
                                                                                            :]

        d_raw_trajectory = np.zeros_like(raw_trajectory)
        d_zv_shaped_trajectory = np.zeros_like(raw_trajectory)
        d_zvd_shaper_trajectory = np.zeros_like(raw_trajectory)
        d_no_delay_shaped_trajectory = np.zeros_like(raw_trajectory)
        dd_raw_trajectory = np.zeros_like(raw_trajectory)
        dd_zv_shaped_trajectory = np.zeros_like(raw_trajectory)
        dd_zvd_shaper_trajectory = np.zeros_like(raw_trajectory)
        dd_no_delay_shaped_trajectory = np.zeros_like(raw_trajectory)

        for i in range(raw_trajectory.shape[1]):
            d_raw_trajectory[:, i] = np.gradient(raw_trajectory[:, i], times)
            d_zv_shaped_trajectory[:, i] = np.gradient(zv_shaped_trajectory[:, i], times)
            d_zvd_shaper_trajectory[:, i] = np.gradient(zvd_shaper_trajectory[:, i], times)
            d_no_delay_shaped_trajectory[:, i] = np.gradient(no_delay_shaped_trajectory[:, i], times)

            dd_raw_trajectory[:, i] = np.gradient(d_raw_trajectory[:, i], times)
            dd_zv_shaped_trajectory[:, i] = np.gradient(d_zv_shaped_trajectory[:, i], times)
            dd_zvd_shaper_trajectory[:, i] = np.gradient(d_zvd_shaper_trajectory[:, i], times)
            dd_no_delay_shaped_trajectory[:, i] = np.gradient(d_no_delay_shaped_trajectory[:, i], times)

        plt.figure(1)
        for i in range(3):
            plt.subplot(3, 1, i + 1)
            plt.plot(times, raw_trajectory[:, i], '-', label='raw trajectory', linewidth=1.2)
            plt.plot(times, zv_shaped_trajectory[:, i], '--', label='zv shaped trajectory', linewidth=1.2)
            plt.plot(times, zvd_shaper_trajectory[:, i], '-.', label='zvd shaped trajectory', linewidth=1.2)
            plt.plot(times, no_delay_shaped_trajectory[:, i], ':', label='no delay shaped trajectory', linewidth=1.2)
            plt.legend()
        plt.tight_layout()
        ...
        for i in range(3):
            plt.subplot(3, 1, i + 1)
            plt.plot(times, d_raw_trajectory[:, i], '-', label='raw trajectory', linewidth=1.2)
            plt.plot(times, d_zv_shaped_trajectory[:, i], '--', label='zv shaped trajectory', linewidth=1.2)
            plt.plot(times, d_zvd_shaper_trajectory[:, i], '-.', label='zvd shaped trajectory', linewidth=1.2)
            plt.plot(times, d_no_delay_shaped_trajectory[:, i], ':', label='no delay shaped trajectory', linewidth=1.2)
            plt.legend()
        plt.tight_layout()
        ...
        for i in range(3):
            plt.subplot(3, 1, i + 1)
            plt.plot(times, dd_raw_trajectory[:, i], '-', label='raw trajectory', linewidth=1.2)
            plt.plot(times, dd_zv_shaped_trajectory[:, i], '--', label='zv shaped trajectory', linewidth=1.2)
            plt.plot(times, dd_zvd_shaper_trajectory[:, i], '-.', label='zvd shaped trajectory', linewidth=1.2)
            plt.plot(times, dd_no_delay_shaped_trajectory[:, i], ':', label='no delay shaped trajectory', linewidth=1.2)
            plt.legend()
        plt.tight_layout()

        plt.show()

    def test_no_delat_shaper(self):
        open_shaper = True
        ...
        omega_d = 2 * np.pi / Td
        zv_shaper = ZVShaper(omega_d, zeta, model.opt.timestep)
        zvd_shaper = ZVDShaper(omega_d, zeta, model.opt.timestep)
        no_delay_shaper = NoDelayShaper(zv_shaper)
