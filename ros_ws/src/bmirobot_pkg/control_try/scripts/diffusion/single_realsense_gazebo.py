...
import cv2
from threadpoolctl import threadpool_limits   # number thread
from multiprocessing.managers import SharedMemoryManager
from diffusion.timestamp_accumulator import get_accumulate_timestamp_idxs
from diffusion.shared_ndarray import SharedNDArray
from diffusion.shared_memory_ring_buffer import SharedMemoryRingBuffer
from diffusion.shared_memory_queue import SharedMemoryQueue, Full, Empty
from diffusion.video_recorder import VideoRecorder
...
class SingleRealsense(mp.Process):
    ...
    def __init__(
            ...
            verbose=False,
            #color_data1 = None,   # add  rgb
            #get_timestamp = 0       # add time
            child_conn = None
        ):
        super().__init__()
        ...
        vis_ring_buffer = SharedMemoryRingBuffer.create_from_examples(
            ...
            examples=examples if vis_transform is None 
                else vis_transform(dict(examples)),    # vis_transform
            get_max_k=1,
            get_time_budget=0.2,
        ...
        ring_buffer = SharedMemoryRingBuffer.create_from_examples(
            ...
            examples=examples if transform is None
                else transform(dict(examples)),       # transform
            get_max_k=get_max_k,
            get_time_budget=0.2,
        ...
        examples = {
            'cmd': Command.SET_COLOR_OPTION.value,
             #'option_enum': rs.option.exposure.value,    # include option_enum (rs), delete
            'option_value': 0.0,
        ...
        # copied variables
        self.get_timestamp = 0  # need to be change
        ...
        self.recording_transform = recording_transform  # None
    ...
    @staticmethod
    def get_connected_devices_serial():   # publish the number
        num = ['0','1']
        serials = list(num)
    ...
    def start(self, wait=True, put_start_time=None):
        ...
        self.thread = mp.Process(target=self.run)    # start
        super().start()
        if wait:
            self.start_wait()
    ...
    def end_wait(self):
        self.join()
        if self.thread is not None and self.thread.is_alive():    # multiprocess
        	self.thread.join()
    ...
    # ========= user API ===========
    def set_color_option(self, value: float):     #  rgb_raw,option  (option: rs.option, value: float)
    ...
    def set_exposure(self, exposure=None, gain=None):    # !! delete:color
        """
        ...
        if exposure is None and gain is None:
            # auto exposure
            self.set_color_option(1.0)
        else:
            # manual exposure
            self.set_color_option(0.0)
            if exposure is not None:
                self.set_color_option(exposure)
            if gain is not None:
                self.set_color_option(gain)
    
    def set_white_balance(self, white_balance=None):
        if white_balance is None:
            self.set_color_option(1.0)
        else:
            self.set_color_option(white_balance)
    ...
    def run(self):
        ...
        #align = rs.align(rs.stream.color)    # align color with depth  !!
        ...
        try:
            ...
            #d = pipeline_profile.get_device().first_color_sensor()      # simultaneous
            ...
            # get
            #color_stream = pipeline_profile.get_stream(rs.stream.color)          #  !!  get the stream of color, depth
            ...
            while not self.stop_event.is_set():
                ...
                data['camera_capture_timestamp'] = self.get_timestamp  # topic  !!
                #print(frame)
                #print(self.get_timestamp)
                if self.enable_color:
                    #color_frame = frameset.get_color_frame()        # about color/image !!
                    data['color'] = np.asarray(frame['data'])       # !!    set two option: if it is 1,get data1; if it is 2,get data2;
                ...
                if self.put_downsample:                
                    ...
                    put_data['timestamp'] = receive_time
                    self.ring_buffer.put(put_data, wait=False)
                    time.sleep(0.1)       # set the frequency of the ring
