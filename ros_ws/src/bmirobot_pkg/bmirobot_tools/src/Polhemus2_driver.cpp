...
void analysis(uint8_t* rxline)
{
    ...
    float tempvalue;
	char* value;
	if((rxline[0]==0x30)&&(rxline[2]==0x20))
	{
		index=rxline[1]-0x31;
		...
		for(int j=0;j<6;j++)
		{
			...
			value[0]=(rxline[3+j*4]);
			value[1]=(rxline[3+j*4+1]);
			value[2]=(rxline[3+j*4+2]);
			value[3]=(rxline[3+j*4+3]);
...
int main(int argc, char **argv)
{
	...
	while(ros::ok())
	{
		...
		rxcnt=my_serial.read(rxdata,29*1);
