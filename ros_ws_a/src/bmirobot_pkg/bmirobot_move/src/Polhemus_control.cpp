...
Eigen::Matrix3d homeMat0,homeMat0to1,homeMat1to2,homeMat2to3,homeMat3to4;
...
void homeEular()
{
	...
	initjoint1=	Eigen::AngleAxisd(-pi, Eigen::Vector3d(0, 0, 1))
        	  * Eigen::AngleAxisd(pi/2, Eigen::Vector3d(0, 1, 0)) 
        	  * Eigen::AngleAxisd(0, Eigen::Vector3d(1, 0, 0));
	
	initjoint2=Eigen::AngleAxisd(0, Eigen::Vector3d(0, 0, 1))  
	...
	initjoint4=Eigen::AngleAxisd(0, Eigen::Vector3d(0, 0, 1))  
        	   * Eigen::AngleAxisd(0, Eigen::Vector3d(0, 1, 0))  
        	   * Eigen::AngleAxisd(pi/2, Eigen::Vector3d(1, 0, 0)); 
	
	initjoint5=Eigen::AngleAxisd(0, Eigen::Vector3d(0, 0, 1))  
        	   * Eigen::AngleAxisd(0, Eigen::Vector3d(0, 1, 0))  
        	   * Eigen::AngleAxisd(-pi, Eigen::Vector3d(1, 0, 0)); 
	...
	homeMat0	=	MRhome[0]*initjoint1;
	homeMat0to1	=	homeMat0.inverse()*MRhome[0];
	homeMat1to2	=	(MRhome[0]*initjoint2).inverse()*MRhome[1];
	homeMat2to3	=	(MRhome[1]*initjoint3).inverse()*MRhome[2];
	homeMat3to4	=	(MRhome[2]*initjoint4).inverse()*MRhome[3];
	
	Mi2j[0]=homeMat0.inverse()*MRhome[0]*initjoint2;
	Mi2j[1]=(MRhome[0]*initjoint2).inverse()*MRhome[1]*initjoint3;
	Mi2j[2]=(MRhome[1]*initjoint3).inverse()*MRhome[2]*initjoint4;
	Mi2j[3]=(MRhome[2]*initjoint4).inverse()*(MRhome[3]*initjoint5);

	getmatrix();

	ja1[1]=asin(-Mi2j[0](0,0));
	ja1[2]=atan2(Mi2j[0](0,2),-Mi2j[0](0,1));	
	ja1[0]=atan2(Mi2j[0](1,0),-Mi2j[0](2,0));
	printf("homeangle: %12f,%12f,%12f \n",ja1[0],ja1[1],ja1[2]);

	ja2[0]=atan2(-Mi2j[1](1,0),Mi2j[1](2,0));
	ja2[1]=asin(Mi2j[1](0,0));
	ja2[2]=atan2(Mi2j[1](0,2),-Mi2j[1](0,1));
	printf("homeangle: %12f,%12f,%12f \n",ja2[0],ja2[1],ja2[2]);

	ja3[0]=atan2(Mi2j[2](0,2),Mi2j[2](1,2));
	ja3[1]=atan2(-Mi2j[2](2,0),-Mi2j[2](2,1));
	printf("homeangle: %12f,%12f,%12f \n",ja3[0],ja3[1],ja3[2]);

	ja4[0]=atan2(-Mi2j[3](0,1),Mi2j[3](0,0));

	homeangle[0]=ja1[0];
	homeangle[1]=ja1[1];
	homeangle[2]=ja1[2]+ja2[0];
	homeangle[3]=ja2[1];	
	homeangle[4]=ja2[2];
	homeangle[5]=ja3[0];
	homeangle[6]=ja3[1];
...
/*
	Mi2j[0]=homeMat0.inverse()*MRnow[0]*initjoint2;
	Mi2j[1]=homeMat1to2.inverse()*(MRnow[0]*initjoint2).inverse()*MRnow[1]*initjoint3;
	Mi2j[2]=homeMat2to3.inverse()*(MRnow[1]*initjoint3).inverse()*MRnow[2]*initjoint4;
	Mi2j[3]=homeMat3to4.inverse()*(MRnow[2]*initjoint4).inverse()*(MRnow[3]*initjoint5);
*/

	Mi2j[0]=homeMat0.inverse()*MRnow[0]*initjoint2;
	Mi2j[1]=(MRnow[0]*initjoint2).inverse()*MRnow[1]*initjoint3;
	Mi2j[2]=(MRnow[1]*initjoint3).inverse()*MRnow[2]*initjoint4;
...
void inverse_angle()
{
	...
	std::cout<< Mi2j[3] <<endl<<endl;

	jointangle[0]=(ja1[0]-homeangle[0]);
	jointangle[1]=(ja1[1]-homeangle[1]);
	jointangle[2]=(ja1[2]+ja2[0]-homeangle[2]);
	jointangle[3]=(ja2[1]-homeangle[3]);	
	jointangle[4]=(ja2[2]-homeangle[4]);
	jointangle[5]=(ja3[0]-homeangle[5]);
	jointangle[6]=(ja3[1]-homeangle[6]);
...
void pulishtf(void)
{
    ...
    br1.sendTransform(tf::StampedTransform(tf::Transform(mat1, vect1), ros::Time::now(), "right_link1", "Polhemuslink"));
...
int main(int argc, char **argv)
{
	...
	ros::Subscriber sub = n.subscribe("/bmirobot/PointsPR", 1000, PointsPRCallback);
  	grasp_joint_pub = n.advertise<std_msgs::Float64MultiArray>( "/bmirobot/right_group_hand_controller/command", 0 );
  	arm_joint_pub = n.advertise<std_msgs::Float64MultiArray>( "/bmirobot/right_group_controller/command", 0 );
