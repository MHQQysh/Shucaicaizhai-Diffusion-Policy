...
class TimestampObsAccumulator:
    def __init__(self, 
        ...
        self.next_global_idx = 0    # restore
    ...
    def actual_timestamps(self):
        if self.timestamp_buffer is None:
            return np.array([])
        return self.timestamp_buffer[:len(self)]    # time at now
    ...
    def put(self, data: Dict[str, np.ndarray], timestamps: np.ndarray):   # data written
        """
        data:
            key: T,*
        """
