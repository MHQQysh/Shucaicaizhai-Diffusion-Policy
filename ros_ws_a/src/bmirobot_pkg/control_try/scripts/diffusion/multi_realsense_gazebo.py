...
from diffusion.single_realsense_gazebo import SingleRealsense
...
class MultiRealsense:
    def __init__(self,
        serial_numbers: Optional[List[str]]=None,    # numbers !!
        ...
        if shm_manager is None:
            ...
            shm_manager.start()
        if serial_numbers is None:   # change !!   
            serial_numbers = SingleRealsense.get_connected_devices_serial()
            print("serial_number",serial_numbers)
        n_cameras = len(serial_numbers)    # the number !!

        advanced_mode_config = repeat_to_list(
            advanced_mode_config, n_cameras, dict)   # advanced setting
        ...
        child_conn  = [child_conn, child_conn_1]
        cameras = dict()
        for i, serial in enumerate(serial_numbers):
            cameras[serial] = SingleRealsense(
                ...
                verbose=verbose,
                child_conn = child_conn[i]
                #color_data1 = color_data1,   # add rgb
                #get_timestamp = get_timestamp      # add time
            )
    ...
    def set_color_option(self, option, value):
        ...
        for i, camera in enumerate(self.cameras.values()):
            camera.set_color_option(value[i])
